...
int main(){
    ...
    // We shift to make sure the color byte of interest is in the lower index byte position so that we can interprete
    // that as an integer, which will be between 0 and 255.

    // Set some format options
    std::cout << std::dec << std::showbase << std::endl;
    
    std::cout << "Red is: " << ((my_color & red_mask) >> 24) << std::endl;
    std::cout << "Greed is: " << ((my_color & green_mask) >> 16) << std::endl;
